...
The preregistered code for analysis (not including MICE) is here: https://osf.io/5jf9p/. Note that it misses ‘source registry’ as a variable to adjust for, which we said in the protocol we would do. Also how we will treat each variable is stated in the data dictionary here: https://osf.io/v63qb/ on pages 23/24. Note that for a few we already said that missing/unclear would be treated as another value because that seems likely to usually be accurate (e.g. unreported blinding we consider to be unblinded).
-->
...
```{r DAG, fig.height = 5, fig.width = 8}
set.seed(12)
# theme_set(theme_dag()) # commented out and added below instead to avoid modifying the theme for subsequent plots
...
# DAG <- ggdag::dagify(outcome ~ covid,
#                      mediators ~ covid,
#                      outcome ~ mediators,
#                      outcome ~ sponsor,
#                      covid ~ sponsor,
...
#                      coords = coords)
# 
# p <- ggdag::ggdag(DAG, text = FALSE, use_labels = "label") + ggdag::theme_dag()
# plot(p)
```
...
```{r}
# paste(names(d)[4:length(names(d))], collapse = " + ")
...
# tableone::CreateTableOne(vars = vars, strata = "trt", data = d, factorVars = factorVars)
```
...
main_direct_adjustment <- main_dataset %>%
  ...
  # select(-source_registry, -conventional) %>%
#  select(covid, source_registry:traditional, -source_registry) %>%
#  select(covid, source_registry:traditional) %>%
  names()
...
# lapply(main_direct_models, summary)
...
# make `covid` factor otherwise it is not used in imputation by default - may be because it is boolean
# main_for_mice$covid <- as.factor(main_for_mice$covid)
...
# sjPlot::tab_model(main_direct_models_mice)
# sjPlot::plot_models(main_direct_models_mice, prefix.labels = "varname") + theme_bw()
```
...
# make `covid` factor otherwise it is not used in imputation by default - may be because it is boolean
# indication_for_mice$covid <- as.factor(indication_for_mice$covid)
# it wasn't because of this...
...
# sjPlot::tab_model(indication_total_models_mice)
# sjPlot::plot_models(indication_total_models_mice, prefix.labels = "varname") + theme_bw()
```
...
result1 <- result
result1[3:6] <-lapply(result[3:6], signif, digits=3)

result1$x <- paste0(result1$Estimate, " [", 
       result1$Lower.CI, "-", result1$Upper.CI, "]")
...
<!-- # # convergence issues with source registry, so group infrequent categories -->
<!-- # d2 <- d2 %>%  -->
<!-- #   mutate(source_registry = fct_lump_n(d2$source_registry, n = 6)) -->
<!-- #  -->
<!-- # summary(d2) -->
<!-- #  -->
<!-- # ``` -->
